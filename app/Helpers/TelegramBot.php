 ...
 * Polling: cron-job.org gọi /cron.php mỗi phút
...
class TelegramBot {
    ...
    private static function offsetFile() {
        if (!self::$offsetFile) {
            self::$offsetFile = defined('APP_DIR')
                ? APP_DIR . '/storage/telegram_offset.txt'
                : __DIR__ . '/../../storage/telegram_offset.txt';
        }
        return self::$offsetFile;
    }

    // Đọc offset — ưu tiên file, fallback DB (để không mất khi Render redeploy)
    private static function getOffset() {
        $f = self::offsetFile();
        if (file_exists($f)) {
            $val = (int)file_get_contents($f);
            if ($val > 0) return $val;
        }
        try {
            $db  = Database::getInstance();
            $row = $db->fetch(
                "SELECT new_data FROM action_logs WHERE action='tg_offset' ORDER BY id DESC LIMIT 1"
            );
    ...
    // Lưu offset — lưu cả file lẫn DB
    private static function saveOffset($offset) {
        $f = self::offsetFile();
        $dir = dirname($f);
        ...
        try {
            ...
            $db->query(
                "INSERT INTO action_logs (user_name, user_role, action, table_name, target_id, new_data, ip_address)
                 VALUES ('TelegramBot', 0, 'tg_offset', 'bot', 0, ?, '127.0.0.1')",
                array((string)$offset)
            );
            // Giữ tối đa 5 bản ghi offset
            $db->query(
                "DELETE FROM action_logs WHERE action='tg_offset'
                 AND id NOT IN (SELECT id FROM (SELECT id FROM action_logs WHERE action='tg_offset' ORDER BY id DESC LIMIT 5) t)"
            );
    ...
    public static function poll() {
        ...
        foreach ($data['result'] as $update) {
            self::saveOffset((int)$update['update_id'] + 1);

            // Bỏ qua nếu không có text (ảnh, sticker, v.v.)
    ...
    private static function describeAction($text) {
        ...
        if (in_array($t, array('start','help','giúp','giup','menu','?','trợ giúp')))
            return array('icon'=>'📋','label'=>'Xem danh sách lệnh');
        if (in_array($t, array('thống kê','thong ke','stats','dashboard','tk')))
            return array('icon'=>'📊','label'=>'Thống kê tổng quan');
        if (strpos($t,'doanh thu')!==false || $t==='revenue' || $t==='dt')
            return array('icon'=>'💰','label'=>'Doanh thu');
        if (strpos($t,'báo cáo')!==false || strpos($t,'bao cao')!==false || $t==='report')
            return array('icon'=>'🤖','label'=>'Báo cáo AI');
        if (preg_match('/(xác nhận|xac nhan|confirm)\s*(đơn|don)?\s*#?(\d+)/iu', $text, $m))
            return array('icon'=>'✅','label'=>'Xác nhận đơn #'.end($m));
        if (preg_match('/(hủy|huy|cancel)\s*(đơn|don)?\s*#?(\d+)/iu', $text, $m))
            return array('icon'=>'❌','label'=>'Hủy đơn #'.end($m));
        if (preg_match('/(đơn|don|order)\s*#?(\d+)/iu', $text, $m))
            return array('icon'=>'🔍','label'=>'Chi tiết đơn #'.$m[2]);
        if (strpos($t,'đơn')!==false || strpos($t,'don hang')!==false || $t==='orders')
            return array('icon'=>'🛒','label'=>'5 đơn hàng mới');
        if (strpos($t,'tồn kho')!==false || strpos($t,'ton kho')!==false || $t==='stock')
            return array('icon'=>'📦','label'=>'Tồn kho thấp');
        if (strpos($t,'sản phẩm')!==false || strpos($t,'san pham')!==false || $t==='products')
            return array('icon'=>'🏷️','label'=>'Danh sách sản phẩm');
        if (strpos($t,'khách hàng')!==false || strpos($t,'khach hang')!==false || $t==='customers')
            return array('icon'=>'👤','label'=>'Khách hàng mới');
        if (preg_match('/^(khóa|khoa)\s+/iu', $text))
            return array('icon'=>'🔒','label'=>'Khóa tài khoản');
        if (preg_match('/^(mở khóa|mo khoa|mở|mo)\s+/iu', $text))
            return array('icon'=>'🔓','label'=>'Mở khóa tài khoản');
    ...
    private static function dispatch($chatId, $text) {
        ...
        // ── Xác nhận đơn: PHẢI check trước order detail ──
        if (preg_match('/(xác nhận|xac nhan|confirm)\s*(đơn|don)?\s*#?(\d+)/iu', $text, $m))
            return self::cmdConfirmOrder($chatId, (int)end($m));

        // ── Hủy đơn: PHẢI check trước order detail ──
        if (preg_match('/(hủy|huy|cancel)\s*(đơn|don)?\s*#?(\d+)/iu', $text, $m))
            return self::cmdCancelOrder($chatId, (int)end($m));

        // Chi tiết đơn: "đơn 123" / "order 123"
        if (preg_match('/(đơn|don|order)\s*#?(\d+)/iu', $text, $m))
        ...
        if (strpos($t,'tồn kho') !== false || strpos($t,'ton kho') !== false
            || strpos($t,'hàng thấp') !== false || $t === 'stock')
    ...
    private static function cmdStats($chatId) {
        try {
            $db   = Database::getInstance();
            $rev  = $db->fetch("SELECT COALESCE(SUM(total),0) AS t FROM orders WHERE DATE(created_at)=CURDATE() AND status NOT IN ('cancelled') AND is_deleted=0");
            $ord  = $db->fetch("SELECT COUNT(*) AS t FROM orders WHERE DATE(created_at)=CURDATE() AND is_deleted=0");
            $pend = $db->fetch("SELECT COUNT(*) AS t FROM orders WHERE status='pending' AND is_deleted=0");
            ...
            // Đếm hàng sắp hết: ưu tiên inventory, fallback products.stock
            $low  = $db->fetch(
                "SELECT COUNT(*) AS t FROM products p
                 LEFT JOIN inventory i ON p.id=i.product_id
                 WHERE p.is_deleted=0 AND p.is_active=1
                 AND COALESCE(i.stock_quantity, p.stock, 0) <= 5"
            );
    ...
    private static function cmdRevenue($chatId) {
        try {
            $db    = Database::getInstance();
            $today = $db->fetch("SELECT COALESCE(SUM(total),0) AS t, COUNT(*) AS c FROM orders WHERE DATE(created_at)=CURDATE() AND status NOT IN ('cancelled') AND is_deleted=0");
            $week  = $db->fetch("SELECT COALESCE(SUM(total),0) AS t FROM orders WHERE created_at>=DATE_SUB(NOW(),INTERVAL 7 DAY) AND status NOT IN ('cancelled') AND is_deleted=0");
            $month = $db->fetch("SELECT COALESCE(SUM(total),0) AS t FROM orders WHERE YEAR(created_at)=YEAR(NOW()) AND MONTH(created_at)=MONTH(NOW()) AND status NOT IN ('cancelled') AND is_deleted=0");
            $top   = $db->fetchAll(
                "SELECT od.product_name AS name, SUM(od.quantity) AS qty
                 FROM order_details od JOIN orders o ON od.order_id=o.id
                 WHERE DATE(o.created_at)=CURDATE() AND o.is_deleted=0
                 GROUP BY od.product_id, od.product_name ORDER BY qty DESC LIMIT 3"
            );
            $msg = "💰 <b>Doanh thu</b>\n\n"
    ...
    private static function cmdOrders($chatId) {
        try {
            ...
            if (!$rows) return self::reply($chatId, '📭 Chưa có đơn hàng nào.');
    ...
    private static function cmdOrderDetail($chatId, $id) {
        try {
            $db  = Database::getInstance();
            $ord = $db->fetch("SELECT * FROM orders WHERE id=? AND is_deleted=0", array($id));
            ...
            if ($ord['status'] === 'pending') {
                $msg .= "\n💡 <code>xác nhận {$id}</code> | <code>hủy {$id}</code>";
            }
        } catch (Exception $e) {
            $msg = '❌ Lỗi: ' . $e->getMessage();
        }
    ...
    private static function cmdConfirmOrder($chatId, $id) {
        try {
            $db  = Database::getInstance();
            $ord = $db->fetch("SELECT id, status FROM orders WHERE id=? AND is_deleted=0", array($id));
            if (!$ord) return self::reply($chatId, "❌ Không tìm thấy đơn #{$id}");
            if ($ord['status'] !== 'pending') {
    ...
    private static function cmdStock($chatId) {
        try {
            ...
            $rows = $db->fetchAll(
                "SELECT p.name,
                        COALESCE(i.stock_quantity, p.stock, 0) AS qty,
                        COALESCE(i.min_stock, 5) AS min_qty
                 FROM products p LEFT JOIN inventory i ON p.id=i.product_id
                 WHERE p.is_deleted=0 AND p.is_active=1
                 AND COALESCE(i.stock_quantity, p.stock, 0) <= COALESCE(i.min_stock, 5)
    ...
    private static function cmdProducts($chatId) {
        try {
            ...
            if (!$rows) return self::reply($chatId, '📭 Chưa có sản phẩm nào.');
    ...
    private static function cmdCustomers($chatId) {
        try {
            $db    = Database::getInstance();
            $rows  = $db->fetchAll(
                "SELECT fullname, email, created_at FROM users WHERE role=0 ORDER BY created_at DESC LIMIT 5"
            );
            ...
            if (!$rows) return self::reply($chatId, '📭 Chưa có khách hàng nào.');
    ...
    private static function cmdLock($chatId, $query, $lock) {
        if (!$query) {
            ...
            return self::reply($chatId, "⚠️ Nhập tên hoặc email cần {$action}.\nVD: <code>" . ($lock?'khóa':'mở khóa') . " user@email.com</code>");
        }
        try {
            $db = Database::getInstance();
            $u  = $db->fetch(
            ...
            if (!$u) return self::reply($chatId, "❌ Không tìm thấy tài khoản: <b>" . htmlspecialchars($query) . "</b>");
            if ($u['role'] == 1) return self::reply($chatId, "⛔ Không thể khóa tài khoản Admin.");

            $alreadyInState = $lock ? ((int)$u['is_active'] === 0) : ((int)$u['is_active'] === 1);
            if ($alreadyInState) {
                $state = $lock ? 'đã khóa rồi' : 'đã mở rồi';
                return self::reply($chatId, "ℹ️ Tài khoản <b>{$u['fullname']}</b> {$state}.");
            }
            $db->query("UPDATE users SET is_active=? WHERE id=?", array($lock ? 0 : 1, $u['id']));
            $icon   = $lock ? '🔒' : '🔓';
            $action = $lock ? 'Đã khóa' : 'Đã mở khóa';
            $msg    = "{$icon} <b>{$action}</b> tài khoản: {$u['fullname']} ({$u['email']})";
        } catch (Exception $e) {
            $msg = '❌ Lỗi: ' . $e->getMessage();
        }
        return self::reply($chatId, $msg);
